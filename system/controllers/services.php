        ...
        if($msg == ''){
			...
			if($typebp == 'Limited'){
				...
				$client->sendSync($addRequest
					...
					->setArgument('on-login', '{
:local voucher $user;
:if ([/ip hotspot user find name=$voucher limit-uptime='.$time_limit.''.strtolower($time_unit).']="") do={/ip hotspot user set [find name=$voucher] limit-uptime='.$time_limit.''.strtolower($time_unit).'};
:if ([/system schedule find name=$voucher]="") do={/system schedule add name=$voucher interval='.$validity.''.strtolower($validity_unit).' on-event="/ip hotspot active remove [find user=$voucher]\r\n/ip hotspot user set limit-uptime=5s  [find name=$voucher]\r\n/ip hotspot user set comment=expire [find name=$voucher]\r\n/system schedule remove [find name=$voucher]"}
}')
				...
				$client->sendSync($addRequest
					...
					->setArgument('on-login', '{
:local voucher $user;
:if ([/ip hotspot user find name=$voucher limit-bytes-total='.$data_limit.''.$data_unit.']="") do={/ip hotspot user set [find name=$voucher] limit-bytes-total='.$data_limit.''.$data_unit.'};
:if ([/system schedule find name=$voucher]="") do={/system schedule add name=$voucher interval='.$validity.''.strtolower($validity_unit).' on-event="/ip hotspot active remove [find user=$voucher]\r\n/ip hotspot user set limit-bytes-total=1024  [find name=$voucher]\r\n/ip hotspot user set comment=expire [find name=$voucher]\r\n/system schedule remove [find name=$voucher]"}
}')
				...
				$client->sendSync($addRequest
					...
					->setArgument('on-login', '{
:local voucher $user;
:if ([/ip hotspot user find name=$voucher limit-uptime=""]="") do={/ip hotspot user set [find name=$voucher] limit-uptime="0"};
:if ([/system schedule find name=$voucher]="") do={/system schedule add name=$voucher interval='.$validity.''.strtolower($validity_unit).' on-event="/ip hotspot active remove [find user=$voucher]\r\n/ip hotspot user set comment=expire [find name=$voucher]\r\n/ip hotspot user set limit-uptime=5s  [find name=$voucher]\r\n/system schedule remove [find name=$voucher]"}
}')
