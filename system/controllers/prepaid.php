...
switch ($action) {
	...
	case 'delete-old-voucher':
    $id  = $routes['2'];
	 
	$old = ORM::for_table('tbl_voucher')->where('status',1)->find_many();
	$old->delete();
	r2(U . 'dashboard', 's', $_L['Delete_Successfully']);	
	...
	case 'print-voucher':
		...
		if ($from_id >0 && $planid >0){
			...
			$vc = ORM::for_table('tbl_plans')
				...
				->where_gt('tbl_voucher.id',$from_id)
				->count();
			$vd = ORM::for_table('tbl_plans')
				->join('tbl_routers', array('tbl_plans.routers', '=', 'tbl_routers.name'))
				->find_one();
    ...
    case 'voucher-post':
		...
		// set status=1 for old voucher before create new voucher
			...
			// Perform queries
			mysqli_query($con,"SELECT * FROM tbl_voucher");
			mysqli_query($con,"UPDATE tbl_voucher
								SET status = '1' 
								WHERE status = '0'");
        ...
        if($msg == ''){
			...
			$secret = strtoupper(substr(md5(time().rand(10000,99999)),0,$lengthcode));
			...
			if($login_mode == 'Usernama_Password'){
				$secret = $secret;
			}else{
				$secret = $code;
			}
						...
						if($p['typebp']=="Limited"){
							if($p['limit_type']=="Time_Limit"){
								...
								$client->sendSync($addRequest
									...
									->setArgument('profile', $p['name_plan'])
									->setArgument('password', $secret)
									->setArgument('limit-uptime', $timelimit)
									->setArgument('comment', 'voucher')
									);
    ...
    case 'voucher-delete':
		...
		if ($d){
			...
			$m = ORM::for_table('tbl_routers')->where('name',$d['routers'])->find_one();
			...
			$printRequest->setQuery(RouterOS\Query::where('name', $d['code']));
